...
<body onload="getEmployeeRecord()">
    ...
    <div class="page-container">
        ...
        if (isset($_POST["edit"])) {
            ...
            include "./db.php";
            // Only hash if a new password was entered, otherwise keep existing
            $raw = $_POST["emp_password"];
            $existing = $conn->query("SELECT emp_password FROM employee WHERE emp_id=$emp_id")->fetch_assoc();
            if (password_verify($raw, $existing['emp_password']) || $raw === $existing['emp_password']) {
                $emp_password = $existing['emp_password'];
            } else {
                $emp_password = password_hash($raw, PASSWORD_DEFAULT);
            }
            $stmt = $conn->prepare("UPDATE employee SET emp_name=?,emp_gender=?,emp_dob=?,emp_position=?,emp_address=?,emp_mobile=?,emp_email=?,emp_password=?,emp_joining_date=? WHERE emp_id=?");
            $stmt->bind_param("sssssssssi", $emp_name, $emp_gender, $emp_dob, $emp_position, $emp_address, $emp_mobile, $emp_email, $emp_password, $emp_joining_date, $emp_id);
            $result = $stmt->execute();
            if ($result == TRUE) {
            ?>
                <script>
