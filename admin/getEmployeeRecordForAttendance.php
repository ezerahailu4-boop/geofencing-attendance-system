...
<div class="table-responsive table-striped table-hoverable" style="padding-bottom: 20px;">
  ...
  <table id="myTable" class=" table table-bordered">
    ...
    <tbody>
      ...
      $graceMins = (int)$shift['grace_minutes'];
      ...
      ?>
          ...
          <tr>
            ...
            <td>
              <?php
              $firstIn = $conn->query("SELECT time FROM checkin WHERE userId='$emp_id' AND DATE(time)='$today_date' ORDER BY time ASC LIMIT 1")->fetch_assoc();
              if ($firstIn) {
                  $checkInTime = strtotime($firstIn['time']);
                  $shiftDeadline = strtotime($today_date . ' ' . $shiftStart) + ($graceMins * 60);
                  if ($checkInTime > $shiftDeadline) {
                      $lateBy = round(($checkInTime - strtotime($today_date . ' ' . $shiftStart)) / 60);
                      echo "<span style='background:#e74c3c;color:#fff;padding:3px 8px;border-radius:4px;font-size:12px;'><i class='fa fa-clock-o'></i> Late by {$lateBy} min</span>";
                  } else {
                      echo "<span style='background:#2ecc71;color:#fff;padding:3px 8px;border-radius:4px;font-size:12px;'><i class='fa fa-check'></i> On Time</span>";
                  }
              }
              ?>
            </td>
