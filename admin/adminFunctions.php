...
{
    include "./db.php";
    $result = $conn->query("SELECT * FROM shift_settings WHERE id=1");
    if ($row = $result->fetch_assoc()) return $row;
    return ['shift_start' => '09:00:00', 'grace_minutes' => 10];
}

// getDailyAttendanceLast7Days
function getDailyAttendanceLast7Days()
{
    include "./db.php";
    $data = [];
    for ($i = 6; $i >= 0; $i--) {
        ...
        $sql = "SELECT count(DISTINCT userId) as cnt FROM checkin WHERE DATE(time)='$date'";
        $result = $conn->query($sql);
        $row = $result->fetch_assoc();
        $data[] = ['label' => $label, 'count' => (int)$row['cnt']];
    }
    return $data;
}
