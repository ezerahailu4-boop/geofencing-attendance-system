...
<body>
    ...
    if (isset($_POST["add"])) {
        ...
        include "./db.php";
        $sql1 = $conn->prepare("SELECT emp_id FROM employee WHERE emp_email=?");
        $sql1->bind_param("s", $emp_email);
        $sql1->execute();
    ...
    ?>
            ...
            if (move_uploaded_file($_FILES["emp_image"]["tmp_name"], "./employee/" . $imageName)) {
                $stmt = $conn->prepare("INSERT INTO employee(emp_name,emp_gender,emp_dob,emp_position,emp_address,emp_mobile,emp_email,emp_password,emp_joining_date,image) VALUES(?,?,?,?,?,?,?,?,?,?)");
                $stmt->bind_param("ssssssssss", $emp_name, $emp_gender, $emp_dob, $emp_position, $emp_address, $emp_mobile, $emp_email, $emp_password, $emp_joining_date, $imageName);
                $result = $stmt->execute();
                if ($result == TRUE) {
            ?>
                    <script>
