...
<head>
    <!-- META SECTION -->
    <title>Edit Location - Admin Panel</title>
...
<body onload="getLocationRecord()">
    ...
    <div class="page-container">
        ...
        if (isset($_POST["edit"])) {
            ...
            include "./db.php";
            $stmt = $conn->prepare("UPDATE master SET lat=?,longi=?,address=? WHERE id=?");
            $stmt->bind_param("sssi", $lat, $longi, $address, $location_id);
            $result = $stmt->execute();
            if ($result == TRUE) {
        ?>
                <script>
                    alertify.success("<span style='color:#fff;font-size:15px;'><i class='fa fa-check-circle'></i> Location Detail's Updated!!!</span>");
                </script>
            <?php
            } else {
            ?>
                <script>
                    alertify.error("<span style='color:#fff;font-size:15px;'><i class='fa fa-warning'></i> Location Detail's Not Updated!!!</span>");
                </script>
        <?php
            }
        } #isset  
        ...
        <script>
            function getLocationRecord() {
                let location_id = <?php echo $_GET["location_id"]; ?>;
                $.ajax({
                    url: `getEditLocation.php?location_id=${location_id}`,
                    success: function(data) {
                        $("#editLocation").html(data);
                    }
                });
            }
