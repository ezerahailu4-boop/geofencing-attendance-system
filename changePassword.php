...
<body onload="getAdminProfileRecord()">
    ...
    <div class="page-container">
        ...
        if (isset($_POST["edit"])) {
            ...
            include "./db.php";
            $sql1 = "SELECT emp_password FROM employee WHERE emp_id='$emp_id'";
            $result1 = $conn->query($sql1);
            if ($row = $result1->fetch_assoc()) {
                $currentHash = $row["emp_password"];
                $oldPassValid = password_verify($old_pass, $currentHash) || $old_pass === $currentHash;
                if ($oldPassValid) {
                    $hashed = password_hash($new_pass, PASSWORD_DEFAULT);
                    $stmt = $conn->prepare("UPDATE employee SET emp_password=? WHERE emp_id=?");
                    $stmt->bind_param("si", $hashed, $emp_id);
                    $result = $stmt->execute();
                    if ($result == TRUE) {
        ?>
                        <script>
